...
router.get("/orders/tracking", (req, res) => {
  ...
  let query = `
    SELECT
        dh.ID_DonHang,
        MIN(dh.NgayDat)            AS NgayDat,
        ANY_VALUE(dh.TrangThai)    AS TrangThai,
        ANY_VALUE(dh.TongTien)     AS TongTien,
        ANY_VALUE(dh.MucDoUuTien)  AS MucDoUuTien,
        ANY_VALUE(dh.NgayCapNhat)  AS NgayCapNhat,
        ANY_VALUE(tk.HoTen)        AS TenKhachHang,
        ANY_VALUE(tk.DienThoai)    AS SoDienThoai,
        ANY_VALUE(tk.DiaChi)       AS DiaChiGiaoHang,
...
`;
  ...
  query += ` GROUP BY
            ...
            tk.DienThoai,
            tk.DiaChi
        ORDER BY dh.NgayDat DESC LIMIT ? OFFSET ?`;
...
router.get("/orders/:id(\\d+)/details", (req, res) => {
  const { id } = req.params;
...
router.get("/orders/:id(\\d+)", (req, res) => {
  const { id } = req.params;
...
router.put("/orders/:id(\\d+)/status", (req, res) => {
  const { id } = req.params;
